import{l as E,c as a,s as c}from"./utilidades-1ea7dec2.js";const o=E("carrinho")??{};function y(){document.getElementById("carrinho").classList.add("right-[0px]"),document.getElementById("carrinho").classList.remove("right-[-360px]")}function C(){document.getElementById("carrinho").classList.remove("right-[0px]"),document.getElementById("carrinho").classList.add("right-[-360px]")}function b(){Object.keys(o).length!==0&&(window.location.href="./checkout.html")}function v(){const t=document.getElementById("fechar-carrinho"),e=document.getElementById("abrir-carrinho"),n=document.getElementById("finalizar-compra");t.addEventListener("click",C),e.addEventListener("click",y),n.addEventListener("click",b)}function m(t){delete o[t],c("carrinho",o),i(),g()}function u(t){o[t]++,c("carrinho",o),i(),f(t)}function $(t){if(o[t]===1){m(t);return}o[t]--,c("carrinho",o),i(),f(t)}function f(t){document.getElementById(`quantidade-${t}`).innerText=o[t]}function h(t){const e=a.find(d=>d.id===t),n=document.getElementById("produtos-carrinho"),r=document.createElement("article"),p=["flex","bg-slate-100","rounded-lg","p-1","relative"];for(const d of p)r.classList.add(d);const x=`<button id="remover-item-${e.id}" class="absolute top-0 right-2">
        <i class="fa-brands fa-xing text-red-500 hover:text-slate-800"></i>
    </button>
    <img
    ...
    <div class='flex text-slate-950 items-end absolute bottom-0 right-2 text-lg'>
        <button id='decrementar-produto-${e.id}' class='text-red-600'>-</button>
        <p id='quantidade-${e.id}' class='ml-2'>${o[e.id]}</p>
        <button class='ml-2 text-green-500' id='incrementar-produto-${e.id}'>+</button>
    </div>`;r.innerHTML=x,n.appendChild(r),document.getElementById(`decrementar-produto-${e.id}`).addEventListener("click",()=>$(e.id)),document.getElementById(`incrementar-produto-${e.id}`).addEventListener("click",()=>u(e.id)),document.getElementById(`remover-item-${e.id}`).addEventListener("click",()=>m(e.id))}function g(){const t=document.getElementById("produtos-carrinho");t.innerHTML="";for(const e in o)h(e)}function B(t){if(t in o){u(t);return}o[t]=1,c("carrinho",o),h(t),i()}function i(){const t=document.getElementById("preco-total");let e=0;for(const n in o)e+=a.find(r=>r.id===n).preco*o[n];t.innerText=`Total: $${e}`}function L(){for(const t of a){const e=`<div class='border-solid w-48 m-2 flex flex-col p-2 justify-between shadow-xl shadow-slate-400 rounded-lg group ${t.feminino?"feminino":"masculino"}' id="card-produto-${t.id}">
        <img
